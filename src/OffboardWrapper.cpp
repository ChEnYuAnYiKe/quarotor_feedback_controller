    ...
    // in real
     if(uav_id == ""){
        m_Subscriber.wrapper_vrpn_sub_ = nh.subscribe<geometry_msgs::PoseStamped>("vrpn_client_node/cyy/pose",
                                                       ...
                                                       this);
        m_Subscriber.wrapper_fused_sub_ = nh.subscribe<nav_msgs::Path>("/fused_path",
                                                       10,
...
void OffboardWrapper::fused_pathCallback(const nav_msgs::Path::ConstPtr &msg) {
    nav_msgs::Path wrapper_current_fusedpath_ = *msg;

    wrap_data.wrapper_current_position_(0) = wrapper_current_fusedpath_.poses.back().pose.position.x;
    wrap_data.wrapper_current_position_(1) = wrapper_current_fusedpath_.poses.back().pose.position.y;
}
