...
OffboardWrapper::OffboardWrapper(geometry_msgs::PoseStamped position_setpoint,
                                 geometry_msgs::PoseStamped end_setpoint,
	...
	// Publisher
	m_Publisher.wrapper_local_pos_pub_ =
	    nh.advertise<geometry_msgs::PoseStamped>(
	        uav_id + "/mavros/setpoint_position/local", 10);
	m_Publisher.wrapper_vision_pos_pub_ =
	    nh.advertise<geometry_msgs::PoseStamped>(
	        uav_id + "/mavros/vision_pose/pose", 10);
	m_Publisher.wrapper_attitude_pub_ =
	    nh.advertise<mavros_msgs::AttitudeTarget>(
	        uav_id + "/mavros/setpoint_raw/attitude", 10);
	m_Publisher.wrapper_angluar_acc_pub =
	    nh.advertise<geometry_msgs::Vector3Stamped>(
	        node_id + "/angluar_acceleration", 10);

	m_Publisher.position_setpoint_pub =
	    nh.advertise<geometry_msgs::PoseStamped>(node_id + "/position_setpoint",
	                                             10);
	m_Publisher.velocity_setpoint_pub =
	    nh.advertise<geometry_msgs::TwistStamped>(
	        node_id + "/velocity_setpoint", 10);
	m_Publisher.attitude_setpoint_pub =
	    nh.advertise<geometry_msgs::Vector3Stamped>(
	        node_id + "/attitude_setpoint", 10);
	m_Publisher.attitude_cureuler_pub =
	    nh.advertise<geometry_msgs::Vector3Stamped>(node_id + "/attitude_euler",
	                                                10);
	m_Publisher.wrapper_status_pub =
	    nh.advertise<std_msgs::Bool>(node_id + "/status", 10);
	m_Publisher.path_ref_pub =
	    nh.advertise<nav_msgs::Path>(node_id + "/ref_path", 1);
	m_Publisher.path_fly_pub =
	    nh.advertise<nav_msgs::Path>(node_id + "/fly_path", 1);

	m_Publisher.time_sync_pub =
	    nh.advertise<std_msgs::Bool>(node_id + "/is_ok", 1);
	if (uav_id == "/uav1") {
		m_Publisher.fly_time_pub =
		    nh.advertise<std_msgs::Time>("offb1_node/fly_time", 10);
	}
	m_Publisher.draw_ref_pub =
	    nh.advertise<geometry_msgs::PoseStamped>(node_id + "/ref_point", 10);
	m_Publisher.draw_fly_pub =
	    nh.advertise<geometry_msgs::PoseStamped>(node_id + "/fly_point", 10);

	// debug
	m_Publisher.atti_cmd_kp_pub = nh.advertise<geometry_msgs::Vector3Stamped>(
	    node_id + "/attitude_cmd_kp", 10);
	m_Publisher.atti_cmd_ki_pub = nh.advertise<geometry_msgs::Vector3Stamped>(
	    node_id + "/attitude_cmd_ki", 10);
	m_Publisher.atti_cmd_kd_pub = nh.advertise<geometry_msgs::Vector3Stamped>(
	    node_id + "/attitude_cmd_kd", 10);
...
void OffboardWrapper::getEndPoint() {
	...
	//  end_position_setpoint_.pose.position.x =
	//  stof(content[(row_num)*col_num-2][1]);
	//  end_position_setpoint_.pose.position.y =
	//  stof(content[(row_num)*col_num-2][2]);
	//  end_position_setpoint_.pose.position.z =
	//  stof(content[(row_num)*col_num-2][3]); fin.close();
}
		...
		if ((ros::Time::now() - start_hover_t).toSec() >= 5.0) {
			if (num_of_ok_drone == 1) {
				if (uav_id == "/uav1" && node_1_time_pub_flg == 1) {
					...
					std::cout << "uav 1 fly time:" << fly_time.toSec()
					          << std::endl;
				...
				if (fly_time_from_node_1 - ros::Time::now().toSec() < 0.1 &&
					...
					std::cout << uav_id << "get time:" << fly_time_from_node_1
					          << std::endl;
					printf("enter planning!!\n");

					std::cout << uav_id
					          << "begin time:" << ros::Time::now().toSec()
					          << std::endl;
...
void OffboardWrapper::subscriber() {
	...
	// in uwb-position
	// m_Subscriber.wrapper_attitude_sub_ = nh.subscribe<sensor_msgs::Imu>("/mavros/imu/data", 10, &OffboardWrapper::attitudeCallback, this);
	m_Subscriber.wrapper_attitude_sub_ = nh.subscribe<sensor_msgs::Imu>("/imu", 10, &OffboardWrapper::attitudeCallback, this);
	...
	// nh.subscribe<geometry_msgs::PoseStamped>("/lidar_position",
	//                                                                               10,
	...
	// in real
	if (uav_id == "") {
		m_Subscriber.wrapper_vrpn_sub_ =
		    nh.subscribe<geometry_msgs::PoseStamped>(
		        "vrpn_client_node/cyy/pose", 10,
		        &OffboardWrapper::visualCallback, this);
	} else if (uav_id == "/uav2") {
		m_Subscriber.wrapper_vrpn_sub_ =
		    nh.subscribe<geometry_msgs::PoseStamped>(
		        "vrpn_client_node/jiahao2/pose", 10,
		        &OffboardWrapper::visualCallback, this);
	}
	// in simulator

	m_Subscriber.wrapper_velocity_sub_ =
	    nh.subscribe<geometry_msgs::TwistStamped>(
	        uav_id + "/outer_velocity", 10, &OffboardWrapper::velocityCallback,
	        this);
	m_Subscriber.wrapper_acc_sub_ = nh.subscribe<geometry_msgs::TwistStamped>(
	    uav_id + "/outer_acc", 10, &OffboardWrapper::accCallback, this);
	m_Subscriber.wrapper_rc_state_sub_ = nh.subscribe<mavros_msgs::VFR_HUD>(
	    uav_id + "/mavros/vfr_hud", 10, &OffboardWrapper::rc_state_Callback,
	    this);
	m_Subscriber.time_sync_sub1 = nh.subscribe<std_msgs::Bool>(
	    "offb1_node/is_ok", 10, &OffboardWrapper::timesyncCallback1, this);
	m_Subscriber.time_sync_sub2 = nh.subscribe<std_msgs::Bool>(
	    "offb2_node/is_ok", 10, &OffboardWrapper::timesyncCallback2, this);
	m_Subscriber.time_sync_sub3 = nh.subscribe<std_msgs::Bool>(
	    "offb3_node/is_ok", 10, &OffboardWrapper::timesyncCallback3, this);
	m_Subscriber.time_sync_sub4 = nh.subscribe<std_msgs::Bool>(
	    "offb4_node/is_ok", 10, &OffboardWrapper::timesyncCallback4, this);
	m_Subscriber.time_sync_sub5 = nh.subscribe<std_msgs::Bool>(
	    "offb5_node/is_ok", 10, &OffboardWrapper::timesyncCallback5, this);

	m_Subscriber.fly_time_sub = nh.subscribe<std_msgs::Time>(
	    "offb1_node/fly_time", 10, &OffboardWrapper::flytimeCallback, this);
	arming_client = nh.serviceClient<mavros_msgs::CommandBool>(
	    uav_id + "/mavros/cmd/arming");
	set_mode_client =
	    nh.serviceClient<mavros_msgs::SetMode>(uav_id + "/mavros/set_mode");
	offb_set_mode.request.custom_mode = "OFFBOARD";
	arm_cmd.request.value = true;

	if (uav_id == "/uav0")
		m_Subscriber.wrapper_status_sub = nh.subscribe<std_msgs::Bool>(
		    "/offb_node/status", 10, &OffboardWrapper::statusCallback, this);
	else if (uav_id == "/uav1")
		m_Subscriber.wrapper_status_sub = nh.subscribe<std_msgs::Bool>(
		    "/offb1_node/status", 10, &OffboardWrapper::statusCallback, this);
	else if (uav_id == "/uav2")
		m_Subscriber.wrapper_status_sub = nh.subscribe<std_msgs::Bool>(
		    "/offb2_node/status", 10, &OffboardWrapper::statusCallback, this);
	else if (uav_id == "/uav3")
		m_Subscriber.wrapper_status_sub = nh.subscribe<std_msgs::Bool>(
		    "/offb3_node/status", 10, &OffboardWrapper::statusCallback, this);
	else if (uav_id == "/uav4")
		m_Subscriber.wrapper_status_sub = nh.subscribe<std_msgs::Bool>(
		    "/offb4_node/status", 10, &OffboardWrapper::statusCallback, this);
	else if (uav_id == "/uav5")
		m_Subscriber.wrapper_status_sub = nh.subscribe<std_msgs::Bool>(
		    "/offb5_node/status", 10, &OffboardWrapper::statusCallback, this);
}

void OffboardWrapper::rc_state_Callback(
    const mavros_msgs::VFR_HUD::ConstPtr& msg) {
...
void OffboardWrapper::velocityCallback(
...
void OffboardWrapper::velocityfilteredCallback(
    const geometry_msgs::TwistStamped::ConstPtr& msg) {
	geometry_msgs::TwistStamped wrapper_current_velo_filtered;
	wrapper_current_velo_filtered = *msg;
	Eigen::Vector3d cur_velocity_filtered(
	    wrapper_current_velo_filtered.twist.linear.x,
...
void OffboardWrapper::visualCallback(
	...
	// cur_velosity_ = (cur_position_ - wrap_data.wrapper_last_position_) /
	// (ros::Time::now() - wrap_data.last_v_time).toNSec();
	...
	// std::cout << (ros::Time::now() - wrap_data.last_v_time).toNSec() <<
	// std::endl;
	...
	wrap_data.wrapper_current_orientation(0) =
	    wrapper_current_vrpn_.pose.orientation.x;
	wrap_data.wrapper_current_orientation(1) =
	    wrapper_current_vrpn_.pose.orientation.y;
	wrap_data.wrapper_current_orientation(2) =
...
void OffboardWrapper::positionfilteredCallback(
    const geometry_msgs::PoseStamped::ConstPtr& msg) {
	geometry_msgs::PoseStamped wrapper_current_pos_filtered_ = *msg;

	wrap_data.wrapper_current_position_[0] =
	    wrapper_current_pos_filtered_.pose.position.x;
	wrap_data.wrapper_current_position_[1] =
	    wrapper_current_pos_filtered_.pose.position.y;
	wrap_data.wrapper_current_position_[2] =
	    wrapper_current_pos_filtered_.pose.position.z;
}

void OffboardWrapper::lidarVzCallback(
    const geometry_msgs::PoseStamped::ConstPtr& msg) {
	geometry_msgs::PoseStamped wrapper_current_lidar_vz_ = *msg;
	wrap_data.wrapper_current_position_[2] =
	    wrapper_current_lidar_vz_.pose.position.z;
	wrap_data.wrapper_current_velocity_[2] =
	    wrapper_current_lidar_vz_.pose.orientation.z;
}
...
void OffboardWrapper::attitudeCallback(const sensor_msgs::Imu::ConstPtr& msg) {
	...
	// wrap_data.wrapper_current_orientation(0) =
	// current_imu_status.orientation.x;
	// wrap_data.wrapper_current_orientation(1) =
	// current_imu_status.orientation.y;
	// wrap_data.wrapper_current_orientation(2) =
...
// in real
//  void OffboardWrapper::visualCallback(const
...
void OffboardWrapper::run() {
	...
	while (ros::ok()) {
		// std::cout << "end_position_setpoint_.pose.position.x: " <<
		// end_position_setpoint_.pose.position.x << std::endl; std::cout <<
		// "end_position_setpoint_.pose.position.y: " <<
		// end_position_setpoint_.pose.position.y << std::endl; std::cout <<
		// "end_position_setpoint_.pose.position.z: " <<
		// end_position_setpoint_.pose.position.z << std::endl;
